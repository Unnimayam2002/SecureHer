...
import {
  FaChartBar, FaUser, FaSignal, FaHandsHelping, FaFileAlt, FaBook,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {distressSignals?.map((signal) => (
              <div key={signal._id} className="bg-white rounded-2xl shadow-lg p-6 transform transition duration-500 hover:scale-105 hover:shadow-2xl">
                ...
                <p className={`text-lg font-semibold mt-4 ${signal.status === 'Active' ? 'text-red-500' : signal.status === 'Resolved' ? 'text-green-500' : 'text-yellow-500'}`}>
